...
if ( ! function_exists( 'gutenbergtheme_posted_on' ) ) :
	...
	function gutenbergtheme_posted_on() {
		...
		$posted_on = sprintf(
			...
			'<stong>' . $time_string . '</stong>'
//			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);

		$byline = sprintf(
			/* translators: %s: post author. */
			esc_html_x( ' %s', 'post author', 'gutenbergtheme' ),
			'<br><span class="author vcard">' . esc_html( get_the_author() ) . '</span>'
//			'<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '"><br>' . esc_html( get_the_author() ) . '</a></span>'
		);
